...
3. **Lee `progress/current.md`** para ver en qué estado quedó la última
   sesión.
4. **Pregunta al usuario** si es una feature o un hotfix.
5. **Lanza el flujo:**
   ```
...
- **Los subagentes escriben sus resultados en archivos**, no en chat.
  Tú recibes solo una referencia: `done -> progress/...`.

## Si el repositorio aún no está adoptado

Si ves que los `docs/` aún tienen placeholders `<...>` o las listas
...
"Cómo adoptarlo en un proyecto nuevo" del `README.md`.

## Si te bloqueas

Anota en `progress/current.md` el bloqueo concreto (archivo, comando,
error) y para la sesión. **No improvises workarounds.**
